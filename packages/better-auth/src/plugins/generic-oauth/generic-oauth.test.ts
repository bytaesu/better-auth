...
describe("oauth2", async () => {
	...
	it("should clear oauth_state cookie with attributes when using cookie-based state storage", async () => {
		server.service.once("beforeUserinfo", (userInfoResponse) => {
			userInfoResponse.body = {
				email: "oauth2-cookie-state-clear@test.com",
				name: "OAuth2 Cookie State Clear",
				sub: "oauth2-cookie-state-clear",
				picture: "https://test.com/picture.png",
				email_verified: true,
			};
			userInfoResponse.statusCode = 200;
		...
		// Get the sign-in URL and capture Set-Cookie headers
		let signInSetCookie = "";
		const res = await authClient.signIn.oauth2({
			providerId: "test-cookie-clear",
			callbackURL: "http://localhost:3000/dashboard",
			...
			fetchOptions: {
				onSuccess(context) {
					signInSetCookie = context.response.headers.get("Set-Cookie") || "";
		...
		const signInCookies = parseSetCookieHeader(signInSetCookie);
		const oauthStateCookieName = Array.from(signInCookies.keys()).find((name) =>
			name.includes("oauth_state"),
		);
		expect(oauthStateCookieName).toBeDefined();
		const oauthStateCookie = signInCookies.get(oauthStateCookieName!);
		expect(oauthStateCookie?.path).toBe("/");
		expect(oauthStateCookie?.httponly).toBe(true);
		expect(oauthStateCookie?.samesite).toBe("lax");

		// Simulate the OAuth flow and capture the callback Set-Cookie headers
		let callbackSetCookie = "";
		let location: string | null = null;
		...
		await betterFetch(location, {
			...
			headers,
			onError(context) {
				callbackSetCookie = context.response.headers.get("Set-Cookie") || "";
				cookieSetter(newHeaders)(context);
			},
		});

		// Verify oauth_state cookie is cleared with all original attributes
		// All attributes must match the original cookie for browsers to properly delete it
		const callbackCookies = parseSetCookieHeader(callbackSetCookie);
		const clearedOauthStateCookie = callbackCookies.get(oauthStateCookieName!);
		expect(clearedOauthStateCookie).toMatchObject({
			"max-age": 0,
			path: oauthStateCookie!.path,
			httponly: oauthStateCookie!.httponly,
			samesite: oauthStateCookie!.samesite,
		});
	});
});
